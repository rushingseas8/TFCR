...
public class BlockBranch extends Block implements IItemSelfRegister, IBlockSelfRegister {
    ...
    // A hashmap connecting TreeTypes to their respective BlockBranch objects.
    private static HashMap<TreeType, BlockBranch> branchMap = 
		(HashMap<TreeType, BlockBranch>) Arrays.stream(TreeType.values())
			.collect(
				Collectors.toMap(Function.identity(),
				tree -> new BlockBranch(tree.getName())));
    
    // A list of all BlockBranchs
    public static final Block[] branchList = 
		Arrays.copyOf(branchMap.values().toArray(), branchMap.size(), Block[].class);
    
	public BlockBranch(String name) {
		super(Material.WOOD);
		this.name = name;
		
		setUnlocalizedName("block_branch_" + name);
		setRegistryName(TFCR.MODID, "wood/branches/block_branch_" + name);
    ...
    public static IBlockState get(TreeType type, EnumDiameter diameter) {
    	return branchMap.get(type).getDefaultState().withProperty(DIAMETER, diameter);
    }
